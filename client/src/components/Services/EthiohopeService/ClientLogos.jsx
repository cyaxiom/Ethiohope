...
export default function ClientSection() {
   ...
   return (
      <section
         ref={ref}
         className="relative w-full overflow-hidden py-10 bg-background text-foreground"
      >
         {/* Background gradient */}
         ...
         <div className="absolute top-1/3 w-full overflow-hidden">
            ...
            >
               {[...logosTop, ...logosTop].map((item, i) => (
                  <div
                     key={`top-${i}`}
                     className="flex items-center gap-2 px-6 py-3 bg-card border border-border rounded-2xl min-w-[150px] justify-center flex-shrink-0"
                  >
                     <span className="text-lg text-muted-foreground">{item.icon}</span>
                     <span className="font-medium text-2xl text-muted-foreground">{item.name}</span>
                  </div>
               ))}
            </div>
            <style>{`
