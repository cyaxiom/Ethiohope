      ...
      <div className="text-center max-w-7xl mx-auto px-4">
        ...
        <div className="flex flex-wrap justify-center items-center gap-x-12 gap-y-8 mt-4">
          {companies.map((company) => (
            <div
              key={company}
              className="flex items-center justify-center cursor-pointer transition duration-300 ease-in-out transform hover:scale-105 dark:hover:opacity-100 opacity-70 group"
              title={`Partner: ${company}`}
            >
              <img
                src={companyLogos[company]}
                alt={`${company} logo`}
                className="h-8 sm:h-10 w-auto object-contain filter grayscale transition duration-300 ease-in-out group-hover:grayscale-50 dark:brightness-75 dark:group-hover:brightness-50"
              />
            </div>
          ))}
        </div>
