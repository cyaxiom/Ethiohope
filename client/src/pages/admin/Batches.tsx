    ...
    <div className="space-y-4 sm:space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Batch Management</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="md:hidden divide-y divide-gray-100">
          {isLoading || isFetching ? (
            ...
            <div className="px-4 py-10 text-center text-gray-400">No batches found.</div>
          ) : (
            batches.map((batch: any) => (
              <div key={batch._id} className="p-4 space-y-3">
                <div className="flex items-start gap-3">
                  <div className="w-11 h-11 rounded-lg bg-indigo-100 text-indigo-600 flex items-center justify-center flex-shrink-0">
                    <Users className="w-5 h-5" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2">
                      <p className="font-bold text-gray-900 truncate">{batch.batchName || 'N/A'}</p>
                      <span className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest flex-shrink-0 ${
                        batch.isActive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
                      }`}>
                        {batch.isActive ? 'Active' : 'Inactive'}
                      </span>
                    </div>
                    <p className="text-sm text-gray-600 mt-1 line-clamp-2">{batch.program?.title || '—'}</p>
                    <p className="text-xs text-gray-400 mt-1">
                      {batch.instructor
                        ? `${batch.instructor.firstname} ${batch.instructor.lastname}`
                        : 'Unassigned'}
                      {' · '}
                      {batch.activeEnrollments || 0}/{batch.capacity || 0} students
                    </p>
                    <div className="w-full h-1.5 bg-gray-100 rounded-full mt-2 overflow-hidden">
                      <div
                        className={`h-full rounded-full ${
                          (batch.activeEnrollments || 0) >= (batch.capacity || 0) ? 'bg-red-500' : 'bg-blue-500'
                        }`}
...
const BatchModal: React.FC<{ onClose: () => void, batch?: any }> = ({ onClose, batch }) => {
  ...
  return (
    <div className="fixed inset-0 z-[999] flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4 md:p-8">
      <div className="bg-white rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[92vh] sm:max-h-[85vh]">
