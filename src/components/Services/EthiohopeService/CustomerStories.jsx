...
export default function CustomerStories() {
   ...
   return (
      <section className="relative min-h-screen flex flex-col justify-evenly items-center bg-background overflow-hidden text-foreground px-6 py-16">
         {/* Rotating globe */}
         <div className="absolute inset-0 flex justify-center items-center">
            <motion.img
               ...
               src={largearth}
               alt="Rotating Globe"
               className="w-[700px] opacity-20 object-contain"
            />
         </div>
         ...
         <motion.div
            ...
            className="relative z-10 bg-card/80 backdrop-blur-md border border-border rounded-xl flex flex-col md:flex-row w-full max-w-6xl overflow-hidden"
         >
            {/* Left: image */}
            ...
            <div className="md:w-1/2 p-8 flex flex-col justify-center">
               ...
               <div className="mb-8">
                  <p className="font-semibold text-foreground">{activeStory.name}</p>
                  <p className="text-muted-foreground text-sm">{activeStory.role}</p>
               </div>
               ...
               <div className="grid grid-cols-2 gap-8 text-center md:text-left">
                  <div>
                     ...
                     <p className="text-sm text-muted-foreground">{activeStory.stat1.label}</p>
                  </div>
                  <div>
                     <p className="text-3xl font-bold text-foreground">
                        {activeStory.stat2.value}
                     </p>
                     <p className="text-sm text-muted-foreground">{activeStory.stat2.label}</p>
                  </div>
               </div>
            </div>
            ...
            <button
               onClick={prevStory}
               className="absolute left-4 top-1/2 -translate-y-1/2 bg-primary/30 hover:bg-primary/50 text-white p-3 rounded-full"
            >
               <FaChevronLeft />
            </button>
            <button
               onClick={nextStory}
               className="absolute right-4 top-1/2 -translate-y-1/2 bg-primary/30 hover:bg-primary/50 text-white p-3 rounded-full"
            >
               <FaChevronRight />
            </button>
         </motion.div>
            ...
            {customerStories.map((story, index) => (
               <button
                  ...
                  className={`transition-all ${index === activeIndex
                     ? "opacity-100 scale-110 border-b-2 border-primary"
